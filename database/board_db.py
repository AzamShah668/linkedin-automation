...
# The database file lives beside this module, in database/. It used to sit in
# output/dashboard/ next to generated HTML, which made it look disposable — it is not: it is the
# board mirror the whole pipeline plans from. Kept out of `output/` so a cleanup of generated
# files can never delete it.
DB_PATH = HERE / "board.sqlite3"

# Legacy home. Read-only fallback so anything still holding the old path keeps working, and so a
# machine that has not run the migration is not silently handed an EMPTY board — which would read
# as "no jobs" rather than as an error (D30).
LEGACY_DB_PATH = ROOT / "output" / "dashboard" / "board.sqlite3"
if not DB_PATH.exists() and LEGACY_DB_PATH.exists():  # pragma: no cover - one-time migration path
