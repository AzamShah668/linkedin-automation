...
- [[33-omniroute-stack]] — 🔴 **read before touching `apps/autopilot/free/`.** The OmniRoute stack built **beside** the Claude one, not instead of it (branch `rewrite/omniroute-native`; the Claude stack stays on `rewrite/phase-0`, unchanged and still scheduled). Why six steps are agent sessions and not programs, the `.env` loader bug that had silently killed `freetext.py` in every unattended run, why the CV needs hard validators (the free model produced two banned AI-tells unprompted), and why you must never pin `auto/*` or call an OmniRoute model a fallback
