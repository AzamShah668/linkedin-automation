...
relevant topic file. Cross-link using wiki-links. Keep it distilled — skip cosmetic changes.
...
- [[29-omniroute-gateway]] — **⚠️ read before pointing Claude Code at any gateway.** The OmniRoute toggle (`claude-free.cmd`), the `/v1`-suffix trap that differs between the two callers, why free models are a launcher and not an env var (D42), and why Capsule Hub cannot be wired into a terminal. §5: the sweep that settled it — **1019 models, three that answer** — why the fallback left the gateway for Groq direct, and the day the canary failed a *working* provider (D43)
- [[30-warm-insider-runbook]] — **the procedure for turning an application that reached nobody into a named human.** Executed end to end on Recro. Prove the company is real (D36), find a genuinely warm path or admit there is none (D8), shortlist by role family, **verify employment from `experience` and never the headline**, then draft and stop. Explains why this is a runbook and not a scraper (D41)
- [[31-apply-batch-runbook]] — 🔴 **READ FIRST when the ask is "find jobs and apply".** The whole loop in order: purge stale → discover with **`f_AL=true`** (not `f_EA`, which LinkedIn silently ignores) → load → **SURVEY every form before answering any** → fill the bank → apply → diagnose by hand. Proven 0 → **20 confirmed submissions** in a day. Carries every trap already paid for: the résumé radio list, the location typeahead, the native `<dialog>`, the 4096-token budget, and the clipboard call that hangs the MCP for 74 minutes
- [[32-the-complete-loop]] — 🔴 **READ FIRST for "is the pipeline finished?" or "why did nothing happen?"** The eight ordered steps behind `pipeline.cmd`, and the three things that stay a human's job on purpose. Covers the step that was missing until 2026-08-15 (`outreach.py`, which turns an application that reached nobody into a *named human* and stops at the ✅), the three silent bugs its first live runs shipped — a stranger, then an ex-employee, then dropping the only genuine lead — why `employment()` returns **CURRENT/PAST/UNKNOWN** and never a boolean, the follow-up cadence finally running on true counts, the one browser profile three steps fight over (**exit 21**), and the scheduled-task defaults that are born broken
- [[33-omniroute-stack]] — ✅ **BUILT 2026-08-17. Read before touching `apps/autopilot/free/`.** The OmniRoute stack built **beside** the Claude one, not instead of it (branch `rewrite/omniroute-native`; the Claude stack stays on `rewrite/phase-0`, unchanged and still scheduled). Why six steps are agent sessions and not programs, the `.env` loader bug that had silently killed `freetext.py` in every unattended run, why the CV needs hard validators (the free model produced two banned AI-tells unprompted), and why you must never pin `auto/*` or call an OmniRoute model a fallback. §12: the two bugs the first run could not find, because **a step with no work is not a tested step** — `dm.py` ran green ten times without ever opening a composer, and reported *"not connected"* for a live accept the moment one arrived (**D52**), while Gmail setup demanded a credential that already existed five times over (**D53**)
- [[35-knowledge-system-audit]] — 🔴 **READ FIRST before adding to any knowledge store, or when asking "is all this documentation worth it?"** The 2026-08-17 audit, all numbers measured: there are **five** stores not three (the undocumented one is the auto-injected `~/.claude/.../memory/`); Brain 3 was queried **once** in 164 sessions while its instruction was carried **564 times**; Brain 2 is 95% accurate and doing **two** jobs (254 of its reads are robots executing runbooks); the disease is **duplication, not absence** — 14 pairs recorded the same principle under names sharing no words, two of them written *by me while diagnosing duplication*. Carries the **routing rule** (which store owns which kind of fact) and the recurrence test
- [[36-state-archive]] — the old `07-current-state.md`, verbatim: 3,325 lines, 59 sections, 40 of them scheduled-agent run notes. Kept because it is the reasoning trail, split out because at 2,566 lines "read it each session" was unfollowable and therefore skipped. ⚠️ Its older sections **contradict each other** — they were written months apart as the project changed. Read it as a log, never as a description of the system
- [[34-changelog]] — **the project's dated history, moved out of `CLAUDE.md` on 2026-08-17.** 24 entries from 2026-07-24 on, verbatim. They lived in `CLAUDE.md`, which loads at the start of *every* session including every unattended agent run, so ~7.6k words of history were being carried dozens of times a day to do work that never needed them. Grep here for *why* something is the way it is; the three most recent entries stay in `CLAUDE.md` because those describe how the system behaves now
