...
## Summary
- 780 nodes · 1162 edges · 72 communities detected
- Extraction: 62% EXTRACTED · 38% INFERRED · 0% AMBIGUOUS · INFERRED: 445 edges (avg confidence: 0.5)
...
4. `Handler` - 12 edges
5. `build_packet()` - 11 edges
6. `_scan()` - 10 edges
7. `_row()` - 10 edges
8. `Thread` - 9 edges
9. `InboxReport` - 9 edges
10. `build_card()` - 9 edges
...
## Knowledge Gaps
- **199 isolated node(s):** `Job Hunt Autopilot — the Python runtime that replaces the Claude-Code-as-server`, `The answer bank and the FIELD_MAP — the only legal source of form values.  THE`, `Read a dotted path out of the bank. Returns None for missing OR null — both mean`, `One mappable question.      patterns  regexes matched (case-insensitive) again`, `Answer 'Are you currently located in <city>?' by COMPARING, never by assuming.` (+194 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 54`** (2 nodes): `__init__.py`, `Job Hunt Autopilot — the Python runtime that replaces the Claude-Code-as-server`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 55`** (2 nodes): `create_privatecloud_pdf_carousel.py`, `process_and_create_pdf()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 56`** (2 nodes): `generate_privatecloud_carousel.py`, `generate_slides()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 57`** (2 nodes): `post_privatecloud_to_linkedin.py`, `publish_privatecloud_bundle()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 58`** (2 nodes): `run-pipeline.ps1`, `Say()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 59`** (2 nodes): `sweep-packets.ps1`, `Say()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 60`** (2 nodes): `page-research.js`, `render()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 61`** (2 nodes): `page-slack.js`, `render()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `LinkedInLoggedOut` connect `Community 5` to `Community 1`, `Community 4`, `Community 7`?**
...
- **Are the 9 inferred relationships involving `build_packet()` (e.g. with `find_packet()` and `find_role_packet()`) actually correct?**
  _`build_packet()` has 9 INFERRED edges - model-reasoned connections that need verification._
